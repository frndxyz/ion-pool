...
function SetupForPool(logger, poolOptions, setupFinished){
    ...
    async.parallel([
        ...
        function(callback){
            daemon.cmd('gettokenbalance', [], function(result){
                if (result.error){
                    callback(true);
                    return;
                }
                try {
                    var d = result.data.split('result":')[1].split(',')[3].split(':')[1].split('.')[1].replace('"}]','');
        ...
        async.waterfall([
            ...
            function(callback){
                ...
                redisClient.multi([
                    ...
                    daemon.cmd('getinfo', [], function(info) {
                        if (info.error){
                            logger.error(logSystem, logComponent, 'Error when getting blockchain height ' + JSON.stringify(info.error));
                            callback(true);
                            return;
                        }
                    
                        var height = info[0].response.blocks;

                        rounds =results[1].filter(function(r) {
                            var details = r.split(':');

                            return height > details[2] + 60; //
                        });
            ...
            function(workers, rounds, callback){
                ...
                var batchRPCcommand = rounds.map(function(r){
                    return ['gettokentransaction', [r.txHash]];
                });

                batchRPCcommand.push(['gettokenbalance', ['ion1zvvh22e0dh35pz5h7lyhlwnv8pzpj204kj078ulumdqdsgqwq8cqsveanek',poolOptions.address]]);
                ...
                daemon.batchCmd(batchRPCcommand, function(error, txDetails){
                    ...
                    txDetails.forEach(function(tx, i){
                        ...
                        else if (tx.result.vout == null || (tx.result.vout && tx.result.vout.length === 0)){
                            logger.debug(logSystem, logComponent, 'Daemon reports no details for transaction: ' + round.txHash);
                            round.category = 'kicked';
                            return;
                        }

                        var generationTx = tx.result.vout.filter(function(tx){
                            return tx.scriptPubKey.addresses[0] === poolOptions.address;
                        })[0];


                        if (!generationTx && tx.result.vout.length === 1){
                            generationTx = tx.result.vout[0];
                        }
                        ...
                        round.reward = generationTx.token.value;
...
logger.info(logSystem, logComponent, addressAmounts);
                    ...
                    var params = ['ion1zvvh22e0dh35pz5h7lyhlwnv8pzpj204kj078ulumdqdsgqwq8cqsveanek'];
                    for(k in addressAmounts){
                        params.push(k);
                        params.push(addressAmounts[k]);
                    };

                    daemon.cmd('sendtoken', params, function (result) {
                        ...
                        if (result.error && result.error.code === -6) {
                            ...
                            logger.error(logSystem, logComponent, 'Error trying to send payments with RPC sendtoken ' + JSON.stringify(result.error));
                        }
                        else if (result.error) {
                            logger.error(logSystem, logComponent, 'Error trying to send payments with RPC sendtoken '
